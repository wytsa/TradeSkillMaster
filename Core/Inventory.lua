...
function private.MailThread(self)
	...
	local function OnAuctionCanceled(index)
		local itemString = TSMAPI:GetBaseItemString2(GetAuctionItemLink("owner", index))
	...
	local function OnMailSent(target)
		...
		for i = 1, ATTACHMENTS_MAX_SEND do
			local itemString = TSMAPI:GetBaseItemString2(GetSendMailItemLink(i))
			if itemString then
	...
	local function OnReturnMail(index)
		...
		local items = {}
		for i = 1, ATTACHMENTS_MAX_SEND do
			local itemString = TSMAPI:GetBaseItemString2(GetInboxItemLink(i))
			if itemString then
...
local function PendingMailSort(a, b)
	...
	for i=1, GetInboxNumItems() do
		...
		if hasItem then
			for j=1, ATTACHMENTS_MAX_RECEIVE do
				local itemString = TSMAPI:GetBaseItemString2(GetInboxItemLink(i, j))
				if itemString then
