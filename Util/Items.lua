...
function TSMAPI:IsSoulbound(bag, slot)
	...
	for id=1, scanTooltip:NumLines() do
		...
		if text and ((text == ITEM_BIND_ON_PICKUP and id < 4) or text == ITEM_SOULBOUND or text == ITEM_BIND_QUEST or text == ITEM_ACCOUNTBOUND or text == ITEM_BIND_TO_ACCOUNT) then
			resultsCache[slotID] = {soulbound=true}
			break
		end
	end
